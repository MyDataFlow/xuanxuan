...
const MessageListItem = React.createClass({
    ...
    render() {
        ...
        if(hideAvatar) {
            ...
            headerElement = <div style={STYLE.title}><strong onContextMenu={this._handleMemberItemContextMenu} title={message.sender ? ('@' + (message.sender.realname || message.sender.account)) : ''} style={{color: Theme.color.primary1}} className='link-app message-title' data-target={message.sender ? '@Member/' + (message.sender.realname || message.sender.account) : '#'}>{message.sender ? message.sender.displayName : ('用户<' + message.user + '>')}</strong> <small style={STYLE.time} title={dateStr}>{timeStr}</small></div>;
        }
