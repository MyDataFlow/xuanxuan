...
服务器地址为一个常见的 web 站点地址，通常由 xxd 管理员提供，一般格式为 `https:// xxd所在服务器的ip或域名：端口号`。当端口号是默认的11443时可以省略。注意：xxd 的服务器地址一定是 https 的。如果你使用的是 `v1.0` 旧版本服务器，则服务器地址格式为 `然之协同站点地址#v1.0`。账号和密码为然之协同上对应用户的账号和密码。填写完成后就可以进行登录。
