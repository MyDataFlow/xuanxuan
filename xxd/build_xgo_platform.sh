...
echo "build win64 ..."
xgo -out $folder/xxd.exe --deps=https://gmplib.org/download/gmp/gmp-6.1.2.tar.bz2 --targets=windows/amd64 .
cp -ra config $folder
...
echo "build win32 ..."
xgo -out $folder/xxd.exe --deps=https://gmplib.org/download/gmp/gmp-6.1.2.tar.bz2 --targets=windows/386 .
cp -ra config $folder
zip -rq $folder.zip $folder
rm -rf $folder
